...
- **Audit (report-only) mode** — roll out pinning safely: validate and report mismatches without blocking (`enforcePinning: false`)
- **Pin-failure reporting** — JS listener + HPKP-style `reportUris` POSTs for fleet-wide visibility
- **Pin expiration** — per-domain fail-open `expirationDate` so abandoned installs never brick
- **Signed OTA pin updates** — `updatePinsFromUrl()` with Ed25519-signed bundles; rotate pins without an app release
- **CLI tooling** — `npx react-native-ssl-manager pins/verify/keygen/sign`: extract SPKI pins without openssl, catch pin drift in CI, author signed bundles
...
Extract your pins with the bundled CLI (no openssl incantation needed):
...
Optional per-domain options and failure reporting:
...
| `expirationDate` | none | `YYYY-MM-DD`; after this date pinning for the domain **fails open** on both platforms (circuit breaker for abandoned installs) |
...
### 4. (Optional) Observe pin failures
...
Fires for enforced blocks AND audit-mode (`enforcePinning: false`) mismatches.
Configure `reportUris` to also receive server-side JSON reports with the
served-chain pins (Android) for fleet-wide monitoring.

### 5. (Optional) Rotate pins over the air

Author a signed bundle (CI or a maintainer machine — keep the private key offline):
...
Apply it from the app — the Ed25519 signature is verified before anything changes:
...
`OTA_EXPIRED`, `OTA_ROLLBACK`) and the active config stays untouched. With TLS
certificate lifetimes shrinking to 47 days by 2029, pair this with the CI drift
check below so rotations never strand your installed base.

### 6. (Optional) Catch pin drift in CI

```bash
npx react-native-ssl-manager verify        # exit 1 when an enforced domain's
                                           # live chain matches none of its pins
```

Run it on a schedule (cron/CI): it warns 30 days before a configured
`expirationDate` and prints the served pins whenever a domain drifts.
...
Registers a listener for pin-validation failures (enforced blocks and
audit-mode mismatches). Returns an unsubscribe function. Any number of
listeners can be registered; a listener throwing never affects the others.

### `updatePinsFromUrl(url, options): Promise<OtaResult>`

Fetches an Ed25519-signed pin bundle (authored with the `sign` CLI command),
verifies it against `options.publicKey`, and applies the contained
configuration. Rejects with a coded `OtaError` on any failure — the active
configuration is never touched by an invalid bundle. Options: `publicKey`
(base64, required), `maxAgeMs`, `minIssuedAt`, `fetchFn`.
...
| `verify [--config ssl_config.json]` | Diff live chains against configured pins; exit 1 on enforced drift (CI-friendly); warns 30 days before pin-set expiration |
